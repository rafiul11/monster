<?php
session_start();  
require '../db.php';


$id = $_POST['id'];
$title = $_POST['title'];
$description = $_POST['description'];




if ($_FILES['image']['name'] != '') {
    $upload_file =$_FILES['image'];
    $after_explode= explode('.', $upload_file['name']);
    $extension = end($after_explode);
    $allowed_extension = array('jpg','png','jpeg','gif');
    
        if (in_array($extension, $allowed_extension)) {
            if ($upload_file['size'] < 300000) {
                $select_img = "SELECT * FROM blog WHERE id=$id";
                $select_img_result =mysqli_query($db_connect, $select_img);
                $after_assoc = mysqli_fetch_assoc($select_img_result);
    
                $delete_from = "../upload/blog/".$after_assoc['image'];
                unlink($delete_from);
    
                $update_blog = "UPDATE blog SET title='$title',description='$description' WHERE id=$id";
                $update_blog_result = mysqli_query($db_connect, $update_blog);
    
                $file_name = $id.'.'.$extension;
                $new_location = '../upload/blog/'.$file_name;
                move_uploaded_file($upload_file['tmp_name'], $new_location);
                $update_img = "UPDATE blog SET image='$file_name' WHERE id=$id";
                $update_img_result = mysqli_query($db_connect, $update_img);
    
                $_SESSION['update_blog']='Updated Successfully';
                header('location:blog_edit.php?id='.$id);
            }
             else {
                $_SESSION['size']='File size to high';
                header('location:blog_edit.php?id='.$id);
            }
        } 
        else {
            $_SESSION['extension']='Invalid Extension';
            header('location:blog_edit.php?id='.$id);
        }
    }
    else{
    
        $update_blog = "UPDATE blog SET title='$title',description='$description' WHERE id=$id";
        $update_blog_result = mysqli_query($db_connect,$update_blog);
    
        $_SESSION['update_blog']='Updated Successfully';
        header('location:blog_edit.php?id='.$id);
    }




?>